...
export function applyMove(state: GameState, next: string, need: Record<string, number>, overlap: number): GameState {
  ...
  const historyItem: GameHistoryItem = { word: next.toUpperCase(), spent: need, overlap, ts: Date.now() }
  ...
  for (const h of history) {
    for (const [l, nRaw] of Object.entries(h.spent)) {
      ...
      for (let i = 0; i < puzzleBag.length && n > 0; i++) {
        if (puzzleBag[i] === l && !used.has(i)) { used.add(i); n-- }
      }
    }
  }
  return used
